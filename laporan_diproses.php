...
// Untuk laporan_diproses.php, kita paksa statusnya selalu 'diproses'
$filter_status = 'diproses'; 
$filter_status_lower = 'diproses'; // Tetap 'diproses'
...
// KONDISI PENTING: PAKSA FILTER HANYA UNTUK 'Diproses'
$conditions[] = "p.status = ?";
$params[] = 'Diproses'; // Diubah menjadi 'Diproses'
$param_types .= 's';
...
<head>
    ...
    <title>Laporan Pengaduan Status Diproses - Sistem Suara Kita</title>
    ...
    <style>
        ...
        /* Modifikasi CSS untuk scroll vertikal pada seluruh halaman */
        body {
            font-family: 'Open Sans', sans-serif;
            color: var(--text-dark);
            /* Hapus atau komentari baris yang mengatur flexbox dan min-height 100vh */
            /* min-height: 100vh; */
            /* display: flex; */
            ...
            background-color: rgba(255, 255, 255, 0.8); /* Background untuk body agar teks terlihat */
            /* Jika ingin scrollbar selalu ada, bisa tambahkan: overflow-y: scroll; */
        }
        ...
        .container-report {
            ...
            /* Jika ingin kontainer ini yang di-scroll, uncomment baris di bawah ini: */
            /* max-height: 90vh; */
            /* overflow-y: auto; */
        }
        ...
        /* --- END: CSS for scrollable table --- */

        .table {
            width: 100%;
            border-collapse: collapse;
            margin-top: 20px;
            min-width: 700px; /* Optional: ensures table is at least this wide for scrolling to trigger */
        }
        ...
        @media print {
            ...
            /* Ensure table is not scrollable when printing */
            .table-responsive {
                overflow-x: visible;
            }
            .table {
                min-width: auto; /* Reset min-width for printing */
            }
            .table th, .table td {
    ...
    <div class="container container-report">
        ...
        <div class="report-header no-print">
            <div class="report-header-left">
                ...
                <p>
                    Laporan Pengaduan (Status: **Diproses**
                    <?php
                    $filter_date_info = [];
        ...
        <div class="filter-container no-print mt-4">
            ...
            foreach ($all_filter_options_with_files as $status_option => $filename): 
                ...
                // Tambahkan kondisi untuk 'diproses' dan 'ditolak'
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th>Tanggal Update</th> <th>Lampiran</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php $no = 1; ?>
                        <?php foreach ($data_pengaduan as $row): ?>
                            <tr>
                                ...
                                <td>
                                    <?php 
                                    // Tampilkan tanggal update jika ada dan berbeda dari created_at, jika tidak, tampilkan "Belum Diupdate"
                                    if (!empty($row['updated_at']) && strtotime($row['updated_at']) > strtotime($row['created_at'])) {
                                ...
                                <td>
                                    ...
                                    // Menggunakan operator null coalescing (?? '') untuk memastikan $lampiran_path selalu string
                                    $lampiran_path = isset($row['lampiran_path']) ? $row['lampiran_path'] : ''; 

                                    if (!empty($lampiran_path)) {
                                        $lampiran_path_safe = htmlspecialchars($lampiran_path); // Sanitasi string yang sudah dipastikan tidak null
                                        $file_extension = pathinfo($lampiran_path_safe, PATHINFO_EXTENSION);
                                        $image_extensions = ['jpg', 'jpeg', 'png', 'gif'];
                                        $upload_dir = 'uploads/'; // Direktori tempat lampiran disimpan
                                        $full_path = $upload_dir . $lampiran_path_safe;
        ...
        <?php else: ?>
            <p class="no-data-message">Tidak ada data pengaduan dengan status "Diproses" untuk ditampilkan.</p>
        <?php endif; ?>
        ...
        <div class="text-center mt-4 no-print" style="display:flex; justify-content:center; gap:10px;">
            ...
            <a href="export_excel.php?status=diproses&bulan=<?php echo htmlspecialchars($bulan_pilihan); ?>&tahun=<?php echo htmlspecialchars($tahun_pilihan); ?>" class="btn btn-success">Export ke Excel</a>
