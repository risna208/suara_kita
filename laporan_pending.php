...
// Ambil status dari URL, jika tidak ada atau tidak valid, default ke 'pending' untuk tampilan aktif
$filter_status = $_GET['status'] ?? 'pending'; // DEFAULT KE 'PENDING'

$filter_status_lower = strtolower($filter_status);
// Jika filter yang dipilih dari URL tidak valid, kita defaultkan ke 'pending'
if (!in_array($filter_status_lower, $valid_filter_options)) {
    $filter_status = 'pending';
    $filter_status_lower = 'pending';
}
...
// PENAMBAHAN: p.updated_at ditambahkan di SELECT
...
// KONDISI PENTING: PAKSA FILTER HANYA UNTUK 'Pending'
// Pastikan ini sesuai dengan nama file `laporan_pending.php`
// Jika Anda membuat laporan untuk status lain (misal, laporan_selesai.php),
// maka nilai 'Pending' ini harus disesuaikan.
$conditions[] = "p.status = ?";
$params[] = 'Pending';
$param_types .= 's';
    ...
    <div class="container container-report">
        ...
        <div class="report-header no-print">
            <div class="report-header-left">
                ...
                <p>
                    Laporan Pengaduan (Status: **Pending**
                    <?php
                    $filter_date_info = [];
        ...
        <div class="filter-container no-print mt-4">
            ...
            $all_filter_options_with_files = [
                'semua' => 'cetak_laporan_pengaduan.php', // Asumsi ini file untuk "Semua"
        ...
        <?php if (!empty($data_pengaduan)): ?>
                    ...
                    <tbody>
                        ...
                        <?php foreach ($data_pengaduan as $row): ?>
                            <tr>
                                ...
                                <td>
                                    <?php
                                    // Ensure $row['lampiran_path'] is treated as an empty string if it's null
                                    ...
                                    if (!empty($lampiran_path)) {
                                        ...
                                        echo '<span class="text-muted">Tidak ada lampiran</span>'; // Changed text here
                                    }
                                    ?>
                                </td>
                            </tr>
            ...
            <?php else: ?>
            <p class="no-data-message">Tidak ada data pengaduan dengan status "Pending" untuk ditampilkan.</p>
        <?php endif; ?>
        ...
        <div class="text-center mt-4 no-print" style="display:flex; justify-content:center; gap:10px;">
            ...
            <a href="export_excel.php?status=pending&bulan=<?php echo htmlspecialchars($bulan_pilihan); ?>&tahun=<?php echo htmlspecialchars($tahun_pilihan); ?>" class="btn btn-success">Export ke Excel</a>
